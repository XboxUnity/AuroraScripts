...
function GetWeatherFormat(iniFile)
    ...
    if metric == "" then
        ...
        ::GetMetricFlag::
        local msgData =
            Script.ShowMessageBox("Units", "Which measurement system would you like to use?", "US", "Metric");
        if msgData.Button == 1 then
            metric = "0"
        else
            metric = "1"
    ...
    if needsave == true then
        ...
        if retval == false then
            local msgData = Script.ShowMessageBox("Aurora Weather",
                "There was an error saving your measurement type.\n\nContinue?", "Yes", "No");
            if msgData.Button == 1 then
                goto CleanUp
            end
        end
...
function GetWeatherAPIKey(iniFile)
    ...
    if apiKey == "" then
        apiKey = "";
        ::EnterApiKey::
        local keyboardData = Script.ShowKeyboard("Aurora Keyboard", "Enter your Weather Underground API Key", "",
            KeyboardFlag.Highlight);
        if keyboardData.Canceled == false then
            ...
            if httpData.Success == true then
                ...
                if o.response.error.type == "keynotfound" then
                    local msgData = Script.ShowMessageBox("API Key Invalid",
                        "The API Key entered was not valid.\n\nWould you like to enter another key?.", "Yes", "No");
                    if msgData.Button == 1 then
                        goto EnterApiKey
                    else
                        apiKey = "";
                ...
                local msgData = Script.ShowMessageBox("Aurora Weather",
                    "Download for API Key validation has failed.\n\nTry again?", "Yes", "No");
                if msgData.Button == 1 then
...
function GetWeatherQueryURL(iniFile, apikey)
    ...
    if queryUrl == "" then
        ...
        local keyboardData = Script.ShowKeyboard("Aurora Weather",
            "Enter the name of a location to search. (or advanced location taken from a websearch url, zmw:...)",
