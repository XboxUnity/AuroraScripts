...
-- Aurora fix: precompute these as UTF-8 byte literals instead of calling
-- unicode_codepoint_as_utf8() at module-load time. During Aurora's script-list
-- scan the 'string'/'math' libraries are not available, so running that
-- function here errored ("attempt to index a nil value") and silently kept the
-- script from appearing in the menu. These literals are byte-identical to the
-- original computed values.
local LINE_SEPARATOR_as_utf8      = "\226\128\168" -- U+2028 (E2 80 A8)
local PARAGRAPH_SEPARATOR_as_utf8 = "\226\128\169" -- U+2029 (E2 80 A9)
